DROP FUNCTION IF EXISTS public.complete_pos_sale(uuid, uuid, uuid, jsonb, jsonb, numeric, text, text);
...
DECLARE
  ...
  v_sort integer := 0;
  v_attach boolean := p_invoice_id IS NOT NULL;
...
BEGIN
  ...
  IF v_attach THEN
    SELECT i.id, i.invoice_number, i.status
      INTO v_invoice_id, v_invoice_number, v_status
    ...
    IF NOT FOUND THEN
      RAISE EXCEPTION 'That invoice does not belong to this customer' USING ERRCODE = '22023';
    END IF;

    IF public._invoice_locked(v_invoice_id) OR v_status NOT IN ('draft'::public.billing_status, 'issued'::public.billing_status) THEN
      RAISE EXCEPTION 'That invoice can no longer be changed — ring this up as a separate sale' USING ERRCODE = '22023';
    END IF;

    SELECT coalesce(max(ii.sort_order), -1) + 1 INTO v_sort
  ...
  SELECT i.total, round(coalesce(i.total, 0) - coalesce(i.amount_paid, 0), 2)
    INTO v_invoice_total, v_due
  FROM public.invoices i
  WHERE i.id = v_invoice_id;
  ...
  IF NOT v_attach AND jsonb_array_length(p_tenders) > 0 AND v_paid < v_due THEN
    RAISE EXCEPTION 'Payment is short by R %', to_char(v_due - v_paid, 'FM999999990.00') USING ERRCODE = '22023';
  END IF;

  IF v_paid > v_due THEN
    RAISE EXCEPTION 'Payment amount exceeds the amount due' USING ERRCODE = '22023';
  END IF;
...
REVOKE ALL ON FUNCTION public.complete_pos_sale(uuid, uuid, uuid, jsonb, jsonb, numeric, text, text, uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.complete_pos_sale(uuid, uuid, uuid, jsonb, jsonb, numeric, text, text, uuid) FROM anon;
GRANT EXECUTE ON FUNCTION public.complete_pos_sale(uuid, uuid, uuid, jsonb, jsonb, numeric, text, text, uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.complete_pos_sale(uuid, uuid, uuid, jsonb, jsonb, numeric, text, text, uuid) TO service_role;