...
/**
 * Compact "Waive" control for use inside booking vaccination gate panels,
 * so front-of-house staff can clear a warning without opening the pet record.
 */
export function VaxWaiverQuickAction({ petId, petName }: { petId: string; petName: string }) {
  ...
  async function submit(clear = false) {
    try {
      ...
      toast.success(clear ? `Waiver removed for ${petName}` : `Vaccinations waived for ${petName}`);
      setOpen(false);
    } catch (err: any) {
  ...
  return (
    <>
      <button
        type="button"
        onClick={() => {
          setUntil(q.data?.vax_waived_until ?? plusDays(60));
          if (q.data?.vax_waiver_reason) setReason(q.data.vax_waiver_reason);
          setOpen(true);
        }}
      ...
      {open && (
        <ModalShell
          title={`Waive vaccinations — ${petName}`}
        ...
        >
          <div className="space-y-4 p-5 text-sm">
            <p className="text-xs text-muted-foreground">
              While the waiver is active, {petName} passes every vaccination gate — on this booking and any other.
              Use it for pets whose certificates are on file on paper but not yet uploaded.
            </p>
            {active && (
              <div className="rounded-lg bg-sk-orange-soft px-3 py-2 text-xs text-sk-orange">
                Currently waived until {fmt(q.data?.vax_waived_until)}.
              </div>
            )}
