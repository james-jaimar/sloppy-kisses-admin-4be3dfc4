...
export default function PosSalePanel({
  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-white">
      ...
      <div className="flex shrink-0 items-center justify-between border-b border-border px-3 py-2 xl:px-4 xl:py-3">
        <div>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-2 py-2">
        ...
        {lines.map((l) => {
          ...
          return (
          <div key={l.product.id} className="flex items-center gap-1 rounded-lg px-1 py-1.5 hover:bg-sk-surface-muted/50 xl:gap-2 xl:rounded-xl xl:px-2 xl:py-2">
            <div className="hidden aspect-square h-10 w-10 shrink-0 place-items-center overflow-hidden rounded-lg bg-sk-surface-muted text-xs font-bold text-muted-foreground xl:grid xl:h-11 xl:w-11">
              {img ? (
            ...
            <div className="flex items-center gap-1">
              <button
                ...
                className="grid h-8 w-8 place-items-center rounded-lg border border-border xl:h-9 xl:w-9"
                aria-label="Decrease quantity"
              >
                <Minus className="h-4 w-4" />
              </button>
              <span className="w-6 text-center text-sm font-semibold tabular-nums xl:w-8">{l.qty}</span>
              <button
                onClick={() => onChangeQty(l.product.id, l.qty + 1)}
                className="grid h-8 w-8 place-items-center rounded-lg border border-border xl:h-9 xl:w-9"
            ...
            <div className="w-16 shrink-0 text-right text-xs font-semibold tabular-nums xl:w-20 xl:text-sm">
              R {lineTotal(l).toFixed(2)}
            </div>
            <button
              onClick={() => onRemove(l.product.id)}
              className="grid h-8 w-8 shrink-0 place-items-center rounded-lg text-muted-foreground hover:text-sk-coral-dark xl:h-9 xl:w-9"
      ...
      <div className="shrink-0 space-y-2 border-t border-border bg-white p-3 xl:space-y-3 xl:p-4">
        <button
        ...
        <div className="grid grid-cols-2 gap-2">
          <button
            ...
            disabled={busy || lines.length === 0}
            className="h-10 rounded-lg border border-border text-sm font-semibold disabled:opacity-40 xl:h-12 xl:rounded-xl"
          >
            Cash
          </button>
          <button
